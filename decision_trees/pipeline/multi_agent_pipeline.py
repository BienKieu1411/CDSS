...
def variable_schema() -> dict[str, Any]:
    ...
    variable = {
        ...
        "properties": {
            "id": {"type": "string"},
            "label": {"type": "string"},
            "dataType": {"enum": ["boolean", "integer", "number", "string", "enum", "array"]},
            "unit": {"type": "string"},
...
def merge_variable_proposal(
    ...
    for index, raw in enumerate(proposed):
        ...
        if not re.fullmatch(r"[a-z][a-zA-Z0-9]*(\.[a-zA-Z0-9_]+)*", variable["id"]):
            ...
            continue
        if variable.get("dataType") not in {"boolean", "integer", "number", "string", "enum", "array"}:
