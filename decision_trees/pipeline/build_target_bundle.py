...
def build_variables() -> list[dict[str, Any]]:
    ...
    add(variable("treatment.path", "Nhánh điều trị tối ưu", "enum", None, "derived", s3, "Nhánh được chọn trong sơ đồ điều trị theo encounter và giai đoạn số nhóm thuốc.", allowed=["new_patient_initial_treatment", "new_patient_mandatory_indication", "followup_stage_1_escalate_two_drugs", "followup_two_drugs_controlled", "followup_escalate_three_drugs", "followup_three_drugs_controlled", "followup_escalate_four_drugs", "followup_four_drugs_controlled", "resistant_referral", "review_required"], required=False))
...
def build_trees() -> list[dict[str, Any]]:
    ...
    optimized_nodes = [
        node("optimized_start", "start", "Điều trị tăng huyết áp tối ưu", s3, detail="Phân nhánh theo lần khám và giai đoạn số nhóm thuốc."),
        node("optimized_entry_gate", "condition", "Người lớn >18 tuổi và đã có phân loại HA cần flow điều trị?", s3, logic=all_of(predicate("patient.ageYears", "gt", 18), predicate("bp.category", "in", ["high_normal", "hypertension", "grade1", "grade2", "masked_hypertension"]))),
        node("optimized_encounter_type", "condition", "Đây là bệnh nhân mới (lần khám số 1)?", s3, detail="Lần khám = 1: không kiểm tra lại số lượng thuốc.", logic=predicate("encounter.number", "eq", 1)),
        node("optimized_new_mandatory", "condition", "Bệnh nhân mới có chỉ định điều trị bắt buộc?", s3, logic=predicate("treatment.mandatoryIndication", "eq", True)),
        node("optimized_infer_new_mandatory", "inference", "Bệnh nhân mới — điều trị theo bệnh đồng mắc", s3, data={"resultCode": "new_patient_mandatory_indication", "severity": "high", "sets": {"treatment.path": "new_patient_mandatory_indication"}}),
        ...
        node("optimized_end_four_controlled", "end", "Duy trì 4 thuốc và theo dõi", s3, data={"outcomeCode": "followup_four_drugs_controlled"}),
        node("optimized_end_review", "end", "Chưa đủ dữ liệu chọn giai đoạn điều trị", s3, data={"resultCode": "optimized_treatment_review_required", "outcomeCode": "optimized_treatment_review_required", "sets": {"treatment.path": "review_required"}}),
    ]
    optimized_edges = [
        edge("optimized_start", "optimized_entry_gate"),
        edge("optimized_entry_gate", "optimized_encounter_type", "true", "Đủ điều kiện"), edge("optimized_entry_gate", "optimized_end_review", "false", "Không"),
        edge("optimized_encounter_type", "optimized_new_mandatory", "true", "Bệnh nhân mới — lần 1"), edge("optimized_encounter_type", "optimized_followup_count_valid", "false", "Tái khám — lần >1"),
        edge("optimized_new_mandatory", "optimized_infer_new_mandatory", "true", "Có"), edge("optimized_new_mandatory", "optimized_infer_new", "false", "Không"),
        edge("optimized_infer_new_mandatory", "optimized_end_new_mandatory"), edge("optimized_infer_new", "optimized_end_new"),
        edge("optimized_followup_count_valid", "optimized_previous_count_one", "true", "1–4 thuốc"), edge("optimized_followup_count_valid", "optimized_end_review", "false", "Không hợp lệ"),
        edge("optimized_previous_count_one", "optimized_infer_stage_one", "true", "1 thuốc"), edge("optimized_previous_count_one", "optimized_previous_count_two", "false", "Không phải 1"),
        edge("optimized_infer_stage_one", "optimized_end_stage_one"),
        edge("optimized_previous_count_two", "optimized_control_after_two", "true", "2 thuốc"), edge("optimized_previous_count_two", "optimized_previous_count_three", "false", "Không phải 2"),
        edge("optimized_control_after_two", "optimized_infer_two_controlled", "true", "Đã kiểm soát"), edge("optimized_control_after_two", "optimized_infer_escalate_three", "false", "Chưa kiểm soát"),
        edge("optimized_infer_two_controlled", "optimized_end_two_controlled"), edge("optimized_infer_escalate_three", "optimized_end_escalate_three"),
        edge("optimized_previous_count_three", "optimized_control_after_three", "true", "3 thuốc"), edge("optimized_previous_count_three", "optimized_previous_count_four", "false", "Không phải 3"),
        edge("optimized_control_after_three", "optimized_infer_three_controlled", "true", "Đã kiểm soát"), edge("optimized_control_after_three", "optimized_infer_escalate_four", "false", "Chưa kiểm soát"),
        edge("optimized_infer_three_controlled", "optimized_end_three_controlled"), edge("optimized_infer_escalate_four", "optimized_end_escalate_four"),
        edge("optimized_previous_count_four", "optimized_control_after_four", "true", "4 thuốc"), edge("optimized_previous_count_four", "optimized_end_review", "false", "Không hợp lệ"),
        edge("optimized_control_after_four", "optimized_infer_four_controlled", "true", "Đã kiểm soát"), edge("optimized_control_after_four", "optimized_infer_resistant", "false", "Chưa kiểm soát"),
        edge("optimized_infer_four_controlled", "optimized_end_four_controlled"), edge("optimized_infer_resistant", "optimized_link_resistant"),
    ]
    risk_nodes = [
