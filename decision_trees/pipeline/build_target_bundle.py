...
def build_trees() -> list[dict[str, Any]]:
    ...
    optimized_nodes = [
        ...
        node("optimized_triple_gate", "condition", "Đang sử dụng ≥3 nhóm thuốc?", s3, logic=predicate("medication.agentCount", "gte", 3)),
        node("optimized_bp_controlled_gate", "condition", "Huyết áp lần khám này đã dưới đích từ Cây 2?", s3, detail="HATT < đích HATT và HATTr < đích HATTr.", logic=all_of(field_predicate("bp.assessmentOfficeSystolicMmHg", "lt", "treatment.targetSystolicMmHg"), field_predicate("bp.assessmentOfficeDiastolicMmHg", "lt", "treatment.targetDiastolicMmHg"))),
        node("optimized_infer_triple", "inference", "Đang sử dụng ≥3 nhóm thuốc và HA đã kiểm soát", s3, detail="Tiếp tục theo dõi; không chuyển sang phân loại kháng trị.", data={"resultCode": "triple_combination_controlled", "severity": "high", "sets": {"treatment.path": "triple_combination"}}),
