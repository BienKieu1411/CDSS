#!/usr/bin/env node
"use strict";

/*
 * CDSS local Node.js UI server.
 *
 * Node serves the Figma-aligned UI and delegates clinical decision execution
 * to the Python runtime. The UI surface is intentionally small: load bundle,
 * run one tree, run the linked clinical flow, and serve static assets.
 */

const http = require("node:http");
const fs = require("node:fs");
const path = require("node:path");
const { spawnSync } = require("node:child_process");
const { URL } = require("node:url");

const UI_DIR = __dirname;
const PROJECT_ROOT = path.resolve(UI_DIR, "../..");
const DECISION_DIR = path.join(PROJECT_ROOT, "decision_trees");
const BUNDLE_PATH = path.join(DECISION_DIR, "bundle", "decision_tree_bundle.json");
const MEDICATION_CATALOG_PATH = path.join(DECISION_DIR, "contracts", "antihypertensive_medication_catalog.json");
const ENGINE_PATH = path.join(DECISION_DIR, "runtime", "decision_tree_engine.py");
const PYTHON_PATH = process.env.CDSS_PYTHON || process.env.PYTHON || "python3";
const PUBLIC_DIR = path.join(UI_DIR, "public");
const ICON_DIR = path.join(UI_DIR, "icon");
const MAX_BODY_BYTES = 1024 * 1024;

const VENDOR_ASSETS = {
  "/vendor/cytoscape.min.js": path.join(UI_DIR, "node_modules", "cytoscape", "dist", "cytoscape.min.js"),
  "/vendor/cytoscape-dagre.min.js": path.join(UI_DIR, "node_modules", "cytoscape-dagre", "dist", "cytoscape-dagre.min.js"),
};

const MIME = {
  ".html": "text/html; charset=utf-8",
  ".js": "text/javascript; charset=utf-8",
  ".css": "text/css; charset=utf-8",
  ".json": "application/json; charset=utf-8",
  ".png": "image/png",
  ".svg": "image/svg+xml",
  ".ico": "image/x-icon",
};

function readJson(filePath) {
  return JSON.parse(fs.readFileSync(filePath, "utf8"));
}

function writeJson(filePath, value) {
  fs.writeFileSync(filePath, JSON.stringify(value, null, 2) + "\n", "utf8");
}

function loadBundle() {
  return readJson(BUNDLE_PATH);
}

function getTreeMap(bundle) {
  return Object.fromEntries((bundle.trees || []).map((tree) => [tree.id, tree]));
}

function runPython(scriptPath, args, timeoutMs = 120000) {
  const completed = spawnSync(PYTHON_PATH, [scriptPath, ...args], {
    cwd: DECISION_DIR,
    encoding: "utf8",
    timeout: timeoutMs,
    maxBuffer: 20 * 1024 * 1024,
  });
  if (completed.error) throw completed.error;
  if (completed.status !== 0) {
    const detail = (completed.stderr || completed.stdout || "Python command failed").trim();
    throw new Error(detail.slice(-4000));
  }
  try {
    return JSON.parse(completed.stdout);
  } catch (error) {
    throw new Error(`Python returned invalid JSON: ${error.message}`);
  }
}

function pythonAvailable() {
  const probe = spawnSync(PYTHON_PATH, ["--version"], { cwd: DECISION_DIR, encoding: "utf8" });
  return !probe.error && probe.status === 0;
}

function coerceVariablesForTrees(treeIds, rawVariables, bundle = loadBundle()) {
  if (!rawVariables || typeof rawVariables !== "object" || Array.isArray(rawVariables)) {
    throw new Error("variables must be a JSON object");
  }

  const treeMap = getTreeMap(bundle);
  treeIds.forEach((treeId) => {
    const tree = treeMap[treeId];
    if (!tree) throw new Error(`unknown tree: ${treeId}`);
  });
  const variableMap = Object.fromEntries(bundle.variables.map((variable) => [variable.id, variable]));
  const output = {};

  // Keep every known variable supplied by the client, including outputs from
  // an earlier tree. This is the shared clinical-flow context.
  for (const variableId of Object.keys(rawVariables)) {
    if (!(variableId in variableMap) || rawVariables[variableId] === "" || rawVariables[variableId] == null) continue;
    const variable = variableMap[variableId];
    const value = coerceVariable(variable, rawVariables[variableId]);
    validateVariable(variableId, variable, value);
    output[variableId] = value;
  }

  return output;
}

function requiredVariableIdsForTrees(treeIds, bundle = loadBundle()) {
  const treeMap = getTreeMap(bundle);
  const produced = new Set();
  const required = [];
  for (const treeId of treeIds) {
    const tree = treeMap[treeId];
    if (!tree) throw new Error(`unknown tree: ${treeId}`);
    for (const variableId of tree.inputVariables || []) {
      const variable = bundle.variables.find((item) => item.id === variableId);
      // Derived values are computed by the Python engine and must never be
      // requested as additional manual input fields.
      if (variable?.sourceSystem === "derived") continue;
      if (!produced.has(variableId) && !required.includes(variableId)) required.push(variableId);
    }
    for (const variableId of tree.outputVariables || []) produced.add(variableId);
  }
  return required;
}

function missingRequiredVariables(treeIds, variables, bundle = loadBundle()) {
  return requiredVariableIdsForTrees(treeIds, bundle).filter((variableId) => {
    const value = variables[variableId];
    return value === undefined || value === null || value === "";
  });
}

function needsDataResult(entryTreeId, variables, missingData) {
  return {
    status: "needs_data",
    entryTreeId,
    terminalTreeId: entryTreeId,
    context: variables,
    missingData,
    trace: [],
    decision: { status: "needs_data", missingData },
  };
}

function coerceVariable(variable, raw) {
  if (variable.dataType === "boolean") {
    if (raw === true || String(raw).toLowerCase() === "true") return true;
    if (raw === false || String(raw).toLowerCase() === "false") return false;
    throw new Error(`${variable.id}: expected true or false`);
  }
  if (variable.dataType === "integer") {
    if (!/^-?\d+$/.test(String(raw))) throw new Error(`${variable.id}: expected integer`);
    return Number.parseInt(raw, 10);
  }
  if (variable.dataType === "number") {
    const value = Number(raw);
    if (!Number.isFinite(value)) throw new Error(`${variable.id}: expected number`);
    return value;
  }
  if (variable.dataType === "array") {
    if (Array.isArray(raw)) return raw;
    try {
      const parsed = JSON.parse(String(raw));
      if (Array.isArray(parsed)) return parsed;
    } catch (_) {
      // Accept a convenient comma-separated form for API clients.
    }
    return String(raw).split(/[,;\n\r\t]+/).map((item) => item.trim()).filter(Boolean);
  }
  if (variable.dataType === "string" || variable.dataType === "enum") return String(raw);
  throw new Error(`${variable.id}: unsupported dataType ${variable.dataType}`);
}

function validateVariable(variableId, variable, value) {
  if (variable.allowedValues && !variable.allowedValues.includes(value)) {
    throw new Error(`${variableId}: value ${JSON.stringify(value)} is not in allowedValues`);
  }
  const rules = variable.validation || {};
  if (typeof value === "number") {
    if (rules.minimum != null && value < rules.minimum) throw new Error(`${variableId}: below minimum ${rules.minimum}`);
    if (rules.maximum != null && value > rules.maximum) throw new Error(`${variableId}: above maximum ${rules.maximum}`);
  }
}

function coerceVariables(treeId, rawVariables, bundle = loadBundle()) {
  return coerceVariablesForTrees([treeId], rawVariables, bundle);
}

function runEngine(treeId, rawVariables, options = {}) {
  const bundle = loadBundle();
  const variables = coerceVariables(treeId, rawVariables, bundle);
  if (options.strict) {
    const missingData = missingRequiredVariables([treeId], variables, bundle);
    if (missingData.length) return { variables, result: needsDataResult(treeId, variables, missingData) };
  }
  return runWithInput(["--tree-id", treeId], variables, options.metadata);
}

function runClinicalFlow(startTreeId = "bp_diagnosis", rawVariables = {}, options = {}) {
  const bundle = loadBundle();
  const runtimeStartTreeId = startTreeId;
  const flowTreeIds = [runtimeStartTreeId];
  const variables = coerceVariablesForTrees(flowTreeIds, rawVariables, bundle);
  if (options.strict) {
    const missingData = missingRequiredVariables(flowTreeIds, variables, bundle);
    if (missingData.length) return { variables, result: needsDataResult(runtimeStartTreeId, variables, missingData) };
  }
  return runWithInput(["--flow-start-tree-id", runtimeStartTreeId], variables, options.metadata);
}

function runWithInput(engineArgs, variables, metadata = {}) {
  const tempDir = fs.mkdtempSync(path.join(UI_DIR, ".node-runtime-"));
  const inputPath = path.join(tempDir, "input.json");
  try {
    writeJson(inputPath, { ...metadata, variables });
    const result = runPython(ENGINE_PATH, ["--bundle", BUNDLE_PATH, ...engineArgs, "--input", inputPath]);
    return { variables, result };
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
}

function requestMetadata(payload) {
  const metadata = {};
  for (const key of ["patientId", "encounterId", "contextSnapshotId", "asOf"]) {
    if (payload[key] !== undefined && payload[key] !== null && payload[key] !== "") {
      metadata[key] = String(payload[key]);
    }
  }
  return metadata;
}

function jsonResponse(response, statusCode, value) {
  const body = Buffer.from(JSON.stringify(value, null, 2), "utf8");
  response.writeHead(statusCode, {
    "Content-Type": "application/json; charset=utf-8",
    "Content-Length": body.length,
    "Cache-Control": "no-store",
  });
  response.end(body);
}

function readBody(request) {
  return new Promise((resolve, reject) => {
    let total = 0;
    const chunks = [];
    request.on("data", (chunk) => {
      total += chunk.length;
      if (total > MAX_BODY_BYTES) {
        reject(new Error("request body is too large"));
        request.destroy();
        return;
      }
      chunks.push(chunk);
    });
    request.on("end", () => {
      try {
        resolve(JSON.parse(Buffer.concat(chunks).toString("utf8")));
      } catch (error) {
        reject(new Error(`invalid JSON body: ${error.message}`));
      }
    });
    request.on("error", reject);
  });
}

function serveStatic(response, pathname) {
  const requested = pathname === "/" ? "/index.html" : pathname;
  const filePath = path.resolve(PUBLIC_DIR, `.${requested}`);
  if (!filePath.startsWith(PUBLIC_DIR + path.sep)) return jsonResponse(response, 403, { ok: false, errors: ["Forbidden"] });
  if (!fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) return jsonResponse(response, 404, { ok: false, errors: ["Not found"] });
  const body = fs.readFileSync(filePath);
  response.writeHead(200, {
    "Content-Type": MIME[path.extname(filePath)] || "application/octet-stream",
    "Content-Length": body.length,
    "Cache-Control": "no-store",
  });
  response.end(body);
}

function serveVendor(response, pathname) {
  const filePath = VENDOR_ASSETS[pathname];
  if (!filePath || !fs.existsSync(filePath)) return jsonResponse(response, 404, { ok: false, errors: ["Not found"] });
  const body = fs.readFileSync(filePath);
  response.writeHead(200, { "Content-Type": "text/javascript; charset=utf-8", "Content-Length": body.length, "Cache-Control": "no-store" });
  response.end(body);
}

function serveIcon(response, pathname) {
  const relativePath = pathname.slice("/icon/".length);
  const filePath = path.resolve(ICON_DIR, relativePath);
  if (!filePath.startsWith(ICON_DIR + path.sep)) return jsonResponse(response, 403, { ok: false, errors: ["Forbidden"] });
  if (!fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) return jsonResponse(response, 404, { ok: false, errors: ["Not found"] });
  const body = fs.readFileSync(filePath);
  response.writeHead(200, {
    "Content-Type": MIME[path.extname(filePath)] || "application/octet-stream",
    "Content-Length": body.length,
    "Cache-Control": "no-store",
  });
  response.end(body);
}

async function handle(request, response) {
  const url = new URL(request.url, `http://${request.headers.host || "localhost"}`);
  try {
    if (request.method === "GET") {
      if (url.pathname === "/api/health") return jsonResponse(response, 200, { ok: true, runtime: "node", python: PYTHON_PATH, bundlePath: BUNDLE_PATH });
      if (url.pathname === "/api/bundle") return jsonResponse(response, 200, loadBundle());
      if (url.pathname === "/api/medication-catalog") return jsonResponse(response, 200, readJson(MEDICATION_CATALOG_PATH));
      if (url.pathname.startsWith("/vendor/")) return serveVendor(response, url.pathname);
      if (url.pathname.startsWith("/icon/")) return serveIcon(response, url.pathname);
      return serveStatic(response, url.pathname);
    }

    if (request.method !== "POST") return jsonResponse(response, 405, { ok: false, errors: ["Method not allowed"] });
    const payload = await readBody(request);
    if (!payload || typeof payload !== "object" || Array.isArray(payload)) throw new Error("request body must be a JSON object");
    if (url.pathname === "/api/run") return jsonResponse(response, 200, {
      ok: true,
      ...runEngine(payload.treeId, payload.variables || {}, {
        strict: payload.strict === true,
        metadata: requestMetadata(payload),
      }),
    });
    if (url.pathname === "/api/run-flow") return jsonResponse(response, 200, {
      ok: true,
      ...runClinicalFlow(payload.startTreeId || "bp_diagnosis", payload.variables || {}, {
        strict: payload.strict === true,
        metadata: requestMetadata(payload),
      }),
    });
    return jsonResponse(response, 404, { ok: false, errors: ["Not found"] });
  } catch (error) {
    return jsonResponse(response, 400, { ok: false, errors: [error.message] });
  }
}

function createServer() {
  if (!fs.existsSync(BUNDLE_PATH)) throw new Error(`baseline bundle not found: ${BUNDLE_PATH}`);
  if (!pythonAvailable()) throw new Error(`Python executable is not available: ${PYTHON_PATH}`);
  return http.createServer((request, response) => handle(request, response));
}

function main() {
  const portArg = process.argv.indexOf("--port");
  const port = portArg >= 0 ? Number(process.argv[portArg + 1]) : Number(process.env.PORT || 8501);
  const host = process.env.HOST || "127.0.0.1";
  const server = createServer();
  server.listen(port, host, () => {
    console.log(`CDSS Node UI: http://${host}:${port}`);
    console.log(`Python engine: ${PYTHON_PATH}`);
    console.log(`Bundle: ${BUNDLE_PATH}`);
  });
}

module.exports = {
  BUNDLE_PATH,
  PYTHON_PATH,
  createServer,
  loadBundle,
  coerceVariables,
  coerceVariablesForTrees,
  requiredVariableIdsForTrees,
  missingRequiredVariables,
  runEngine,
  runClinicalFlow,
};

if (require.main === module) main();
